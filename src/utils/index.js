...
function jsGetAge(strBirthday) {
    ...
    if (_diffYear === 0) {
        if (_diffMonth === 0 && _diffDay < 0) {
            ...
            (_diffMonth === 0 && _diffDay >= 0) ||
            (_diffMonth === 1 && _diffDay < 0)
        ) {
            // 不满一月的按未满月展示
            returnAge = '未满月';
        } else {
        ...
        if (+data.type === 1) {
            if (
                ...
                if (data.custom && Object.keys(data.custom).length > 0) {
                    var portStatus = getMultiPortStatus(
                        customType,
                        data.custom
                    );
...
const analysisIDCard = (IDCardNo = '') => {
    ...
    if (IDCardNo.length === 18) {
        ...
        if (
            temp.substring(4, 6) < month ||
            (temp.substring(4, 6) == month && temp.substring(6) <= day)
        ) {
            age++;
        }
    }
